...
request({url: 'https://platzi.com/', encoding: 'utf8' }, function(err, resp, body){
	if(!err && resp.statusCode == 200){
		...
		$('.Career').each(function(){
			...
			Career('.CareerCourse').each(function(){
				...
				var url_cursos = 'https://platzi.com/cursos' + link.slice(7, -1);
				var url_clases = 'https://platzi.com/clases' + link.slice(7, -1);
		...
		fs.writeFileSync("json/platzi-carreras.json", JSON.stringify(carreras));
		...
		for (var i = 0; i < 5 ; i++){
			...
			request({url: cursos[i].url_cursos, encoding: 'utf8'}, function(err, resp, body){
				if (err){
					console.log(err)
					...
					curso_page('.Teacher').each(function(){
						profesor = {
							...
							twitter_link: $(this).find('.Teacher-link').attr('href'),
							label: $(this).find('.Teacher-label').html(),
							figure:  $(this).find('.Teacher-image img').attr('src')
						}
						teachers.push(profesor)

						var profesores = JSON.parse(fs.readFileSync('json/platzi-profesores.json', 'utf8'));
					...
					var cursos = JSON.parse(fs.readFileSync('json/platzi-cursos.json', 'utf8'));			
